...
do
    if [ "$job" == "radix" ]
    then
        RunCommand "$MEMCACHED_EXTERNAL_IP" "docker pull anakli/cca:splash2x_radix" &
    else
        RunCommand "$MEMCACHED_EXTERNAL_IP" "docker pull anakli/cca:parsec_$job" &
    fi
done
wait
...
RunCommand "$CLIENT_MEASURE_EXTERNAL_IP" "~/memcache-perf/mcperf -s $MEMCACHED_INTERNAL_IP --loadonly; tmux new-session -s mcperf -d '~/memcache-perf/mcperf -s $MEMCACHED_INTERNAL_IP -a $CLIENT_AGENT_INTERNAL_IP --noload -T 16 -C 4 -D 4 -Q 1000 -c 4 -t 1800 --qps_interval 10 --qps_min 5000 --qps_max 100000 2>&1 | tee ~/mcperf.txt'"
